...
async function readPublishedDraw() {
  ...
  if (!response.ok)
    throw Error(
      (await response.json()).error ||
        "Não foi possível consultar o sorteio publicado.",
    );
  return (await response.json()).draw;
}
...
async function publishDraw(draw) {
  ...
  if (!response.ok)
    throw Error(
      (await response.json()).error || "Não foi possível publicar o sorteio.",
    );
}

async function unpublishDraw() {
  const response = await fetch("/api/admin/draw", { method: "DELETE" });
  if (!response.ok)
    throw Error(
      (await response.json()).error ||
        "Não foi possível remover o sorteio publicado.",
    );
}
...
function teamIdentity(team, esc, size = "compact") {
  ...
  return logo
    ? `<span class="team-identity has-logo ${size}"><img src="${assetUrl(logo)}" alt="Logo da turma ${esc(team)}"><span class="team-name">${esc(team)}</span></span>`
    : `<span class="team-identity ${size}"><span class="team-name">${esc(team)}</span></span>`;
}

function activateLogoFallback(root) {
  root.querySelectorAll(".team-identity img").forEach((image) => {
    const fail = () => recoverImage(image);
...
function statusFor(competition) {
  if (!competition.participants.length)
    return ["waiting", "Aguardando inscrições"];
  if (competition.participants.length === 1)
    return ["automatic", "Classificação automática"];
...
function collectiveStageMarkup(bracket, state, esc) {
  ...
  const groupMarkup = (group) => {
    ...
    return `<section class="suspense-group group-${group.toLowerCase()}${received}"><div class="group-title"><span>LADO ${group}</span><h3>Grupo ${group}</h3><small>${revealOrder.slice(0, revealed).filter((item) => item.group === group).length}/${bracket.groups[group].length}</small></div><div class="group-slots">${bracket.groups[
      group
    ]
      .map((team) => {
        const revealIndex = revealOrder.findIndex((item) => item.team === team);
        const visible = revealIndex < revealed;
        const hasLogo = Boolean(logoForClass(team));
        return `<span class="group-slot ${visible ? "is-revealed" : "is-hidden"} ${visible && hasLogo ? "with-logo" : ""}"><i>${visible ? "✦" : "?"}</i>${visible ? teamIdentity(team, esc, "group") : "<b>A revelar</b>"}</span>`;
      })
      .join("")}</div></section>`;
  };
...
export function mountDrawAdmin(root, { rows, modalities, esc }) {
  ...
  let drawIsCurrent =
    savedDraw?.version === 3 && savedDraw.signature === signature;

  function renderDrawPanel() {
    const eligible = competitions.filter(
      (competition) => competition.participants.length,
    );
    const ready = competitions.filter(
      (competition) => competition.participants.length >= 2,
    ).length;
    const automatic = competitions.filter(
      (competition) => competition.participants.length === 1,
    ).length;
   ...
   <div class="draw-overview"><div><strong>${eligible.length}</strong><span>competições com inscritos</span></div><div><strong>${ready}</strong><span>chaveamentos para sortear</span></div><div><strong>${automatic}</strong><span>classificações automáticas</span></div><p>${drawIsCurrent ? `Sorteio publicado em ${new Date(savedDraw.generatedAt).toLocaleString("pt-BR")}.` : savedDraw ? "As inscrições mudaram desde o último sorteio. Realize um novo sorteio." : "Nenhum sorteio publicado."}</p></div>
   ...
   <dialog id="draw-presentation" class="draw-presentation"><button class="close" aria-label="Fechar">×</button><div id="draw-slide"></div><footer><button id="previous-slide" class="admin-secondary">← Anterior</button><span id="slide-progress"></span><button id="next-slide" class="submit">Próxima →</button></footer></dialog>`;
    drawPanel.querySelector("#run-draw").onclick = async (event) => {
      if (
        drawIsCurrent &&
        !confirm(
          "Refazer o sorteio substituirá o chaveamento publicado. Continuar?",
        )
      )
        return;
      const button = event.currentTarget,
        nextDraw = createTournamentDraw(competitions, secureRandom);
      button.disabled = true;
      button.textContent = "Publicando…";
      try {
        await publishDraw(nextDraw);
        savedDraw = nextDraw;
        try {
          localStorage.setItem(storageKey, JSON.stringify(savedDraw));
        } catch {}
        drawIsCurrent = true;
        renderDrawPanel();
        openPresentation();
    ...
    drawPanel.querySelector("#clear-draw").onclick = async () => {
      if (
        !savedDraw ||
        !confirm("Limpar o sorteio atual? As inscrições serão mantidas.")
      )
        return;
      try {
        await unpublishDraw();
  ...
  function openPresentation() {
    const eligible = competitions.filter(
      (competition) =>
        ...
        savedDraw?.brackets?.[competition.id],
    );
    ...
    const states = Object.fromEntries(
      eligible.map((competition) => [
        competition.id,
        { revealed: 0, showKnockout: false },
      ]),
    );
    [
      ...new Set(
    ...
    const show = () => {
      ...
      dialog.querySelector("#draw-slide").innerHTML = slideMarkup(
        competition,
        bracket,
        current,
        eligible.length,
      ...
      dialog.querySelector("#slide-progress").textContent =
        `${current + 1} de ${eligible.length}`;
      dialog.querySelector("#previous-slide").disabled =
        current === 0 && !state.showKnockout;
      const next = dialog.querySelector("#next-slide");
      next.disabled = false;
      if (bracket.format === "groups-knockout" && !state.showKnockout) {
        next.textContent =
          state.revealed < bracket.revealOrder.length
            ? `${state.revealed ? "Sortear próxima turma" : "Começar o sorteio"} →`
            : "Ver mata-mata →";
      } else
        next.textContent =
          current === eligible.length - 1 ? "Encerrar" : "Próxima competição →";
    };
    const finishReveal = () => {
      const competition = eligible[current],
        bracket = savedDraw.brackets[competition.id],
        state = states[competition.id];
    ...
    const revealNext = () => {
      ...
      drawTimer = setInterval(() => {
        ...
        const team = dialog.querySelector("#reveal-team"),
          destination = dialog.querySelector("#reveal-destination");
        if (team) team.textContent = candidate.team;
        if (destination)
          destination.textContent =
            tick % 3 === 2 ? `GRUPO ${candidate.group}` : "GRUPO ?";
        tick++;
        if (tick >= 22) {
          ...
          if (team)
            team.innerHTML = teamIdentity(selected.team, esc, "spotlight");
    ...
    dialog.querySelector("#next-slide").onclick = () => {
      ...
      if (bracket.format === "groups-knockout" && !state.showKnockout) {
        if (state.revealed < bracket.revealOrder.length) revealNext();
          ...
          state.showKnockout = true;
          show();
        }
        return;
      }
      if (current === eligible.length - 1) closePresentation();
      else {
        current++;
        show();
      }
    ...
    dialog.onkeydown = (event) => {
      if (event.key === "ArrowRight")
        dialog.querySelector("#next-slide").click();
      if (event.key === "ArrowLeft")
        dialog.querySelector("#previous-slide").click();
    };
    show();
    dialog.showModal();
  ...
  tabs.querySelectorAll("[data-admin-tab]").forEach(
    (button) =>
      (button.onclick = () => {
  ...
  readPublishedDraw()
    .then(async (draw) => {
      if (
        !draw &&
        savedDraw?.version === 3 &&
        savedDraw.signature === signature
      ) {
        await publishDraw(savedDraw);
        draw = savedDraw;
      }
      savedDraw = draw;
      if (draw)
