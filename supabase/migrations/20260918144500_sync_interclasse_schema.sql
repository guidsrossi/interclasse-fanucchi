...
create unique index if not exists interclasse_unique_student
on public.interclasse_registrations(modality_id,class_name,lower(regexp_replace(btrim(student_name),'\s+',' ','g')));
...
language sql security definer set search_path = '' as $$
 select r.modality_id,r.class_name,count(*)
 from public.interclasse_registrations r
 group by r.modality_id,r.class_name;
$$;
...
begin
 ...
 if clean_name is null or char_length(clean_name) not between 3 and 100 then
  raise exception 'Digite um nome entre 3 e 100 caracteres.';
 end if;
 if p_class is null or p_class !~ '^[123]º [A-Z]$' then
  raise exception 'Informe a turma no formato 3º A.';
 end if;
 ...
 if p_modality='atletismo' then
  if p_event is null or p_event not in ('100 m rasos','Salto em distância','Salto em altura','Arremesso de peso') then
   raise exception 'Escolha uma prova válida.';
  end if;
  if p_division is null or p_division not in ('Masculino','Feminino') then
   raise exception 'Escolha a categoria.';
  end if;
 elsif p_event is not null or p_division is not null then
  raise exception 'Esta modalidade não possui provas.';
 end if;
 if exists(
  select 1 from public.interclasse_registrations
  where modality_id=p_modality and class_name=p_class and lower(student_name)=lower(clean_name)
 ) then
  raise exception 'Este aluno já está inscrito nesta modalidade e turma.';
 end if;
 select count(*) into current_count
 from public.interclasse_registrations
 where modality_id=p_modality and class_name=p_class;
 ...
 values(p_modality,clean_name,p_class,p_event,p_division)
