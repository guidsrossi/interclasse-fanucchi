drop index if exists public.interclasse_unique_student;

create unique index interclasse_unique_student
on public.interclasse_registrations(
 modality_id,
 class_name,
 lower(regexp_replace(btrim(student_name),'\s+',' ','g')),
 coalesce(event_name,''),
 coalesce(division,'')
);
...
begin
 ...
 if exists(
  ...
  where modality_id=p_modality
   and class_name=p_class
   and lower(regexp_replace(btrim(student_name),'\s+',' ','g'))=lower(clean_name)
   and event_name is not distinct from case when p_modality='atletismo' then p_event else null end
   and division is not distinct from case when p_modality='atletismo' then p_division else null end
 ) then
 ...
 if p_modality='atletismo' then
  ...
  where modality_id=p_modality and class_name=p_class and event_name=p_event and division=p_division;
 else
  select count(*) into current_count
  from public.interclasse_registrations
