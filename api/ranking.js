...
export default async function handler(req, res) {
  ...
  const [scoresResponse, classesResponse] = await Promise.all([
    fetch(`${url}/rest/v1/interclasse_score_entries?select=id,class_name,modality_id,entry_type,label,points,wins,draws,losses,responsible_student,source,created_at,updated_at&order=updated_at.desc`, { headers: headers(key) }),
